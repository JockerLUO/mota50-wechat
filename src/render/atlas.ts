 ...
 *
 * 2. **`scaleMode` 必须是 `nearest`。** 素材是 16×16 像素画，运行时按 ×2 放大。
...
/**
 * 怪物与角色：**整数倍**。它们的帧是 16×16 / 16×26，落屏 32–52px —— 在这个尺寸上
 ...
 *
 * 为什么这里放宽整数倍：道具帧尺寸本来就不统一（药水 16×16、剑 10×21、金币 8×8），
...
class Atlas {
  ...
  /** 怪物落屏尺寸只由 drawScale 决定（图集里所有怪物帧都已归一化到 16×16） */
