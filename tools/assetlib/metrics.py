...
def _detail_density(im: Image.Image, block: int = 8) -> float:
    ...
    ⚠️ **`block` 必须随网格缩放**：它定义的是「同一块**相对面积**」。
    32 网格的杂兵用 8（= 网格的 1/4），64 网格的 BOSS 也用 8（= 1/8），
    96 网格的 BOSS 用 12（= 1/8）。传错不会报错，只会让两个网格的读数不可比 ——
    块相对面积一变，中位颜色数就跟着变，而变化的量是什么就说不清了。
    约定：**块边长 = 该素材的绘制网格 ÷ 8**（BOSS 与杂兵同口径）。
    """
    px = im.load()
    counts = []
...
def _inner_detail(im: Image.Image, block: int = 4, margin: int = 2) -> float:
    ...
    实测（8 只 BOSS × 两版素材，2026-09-24 重测）：
    改前（64 网格）**1.97 ~ 2.28**，改后（96 网格）**2.47 ~ 2.92**；
    门槛 2.40 —— 旧版八只**全部**低于它，这就是区分力的来源。

    ⚠️ `block` 与 `margin` 同样**随网格缩放**（见 `_detail_density` 的说明）：
    `block` = 网格 ÷ 16，`margin` = 网格 ÷ 32。64 网格 → (4, 2)（旧默认值），
    96 网格 → (6, 3)。腐蚀半径按网格走是必须的：margin 写死 2 时，
    96 网格上那条**1px 的描边**（膨胀只加 1px）会被算进「内部」，
    于是「边缘花哨 + 内部纯色」又能过线了。
    """
...
# 「头部区域」占画布高度的比例 —— **唯一写死的一处**，其余全部按它算。
#
# ⚠️ 原来这里的默认值是**字面量行数** `y1 = 34`，那是 64 网格时代的数。BOSS 网格
# 改到 96 之后，如果继续按 34 去裁，量到的就不是「头」而是「盔顶和一小截脖子」：
# 报出来的不对称度会**变小**（越靠上、成对结构越少），于是这条判据从「正面朝向
# 的守卫」退化成一条几乎恒真的装饰 —— 它不会报红，只会不再保护任何东西。
# 这类「换网格时静默量错区域」的失效比报错难查得多（同 §「断言红了先怀疑期望值」）。
#
# 34/64 = 0.53 是 64 网格时代实测出来的有效区域（含头 + 肩），比例保持不变即可
# 与旧读数直接比：96 网格 → 51 行。
HEAD_BAND_RATIO = 0.53


def _head_asym(im: Image.Image, y1: int | None = None) -> float:
    """
    头部区域（画布最上 `y1` 行）的**左右不对称度** = 不对称像素 / 并集像素。

    `y1` 默认按 `HEAD_BAND_RATIO × 画布高度` 算，**不要传死数字** ——
    调用方一律省略它，让换网格时量到的仍然是同一块相对区域。

    这是「正面朝向玩家」的可量判据。侧视或半侧视的头必然偏在画布一侧，
    怎么修形状都过不了这一条 —— 这正是它存在的理由：没有它，「把龙转成正面」
    只是这一次改对了，下次有人为了「看着更威风」把它转回侧面，没有任何东西会报警。

    实测（64 网格）：改前 dragon 39.4%（侧视朝左）、demonKingTrue 15.1%（角与手臂
    左右各差 1~7 列）、kraken 7.2%、demonKing 9.4%；改后最大 2.3%。
    实测（96 网格）：见 `bosses/common.py` 的 `BOSS_HEAD_SYM_MAX` 注释。
    """
    if y1 is None:
        y1 = max(1, round(im.height * HEAD_BAND_RATIO))
