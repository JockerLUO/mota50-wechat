...
把素材网格从 16 翻到 32（超采样）时，「落屏的**设计**像素数」必须一个都不变
（16×2 = 32×1）。而这两件事**在画面上分不出来**：32 网格按 1:1 画、和
16 网格按 2× 画，落屏一模一样。也就是说，网格翻倍做错了（比如忘了把
`drawScale` 一起减半，版面集体放大一倍），**看起来是好的、版面却是错的**。

所以 A17 同时钉四件事：

1. `rasterTile = baseTile × supersample`（网格真的翻了）；
2. 逐条地形帧 `w = rasterTile` 且 `drawScale = 1`（不再放大）；
3. **拿发布出去的图集帧和第三方原始素材比边长**（`floor_1` 16 → 32）——
...
npm run verify:visual     # 渲染层回归，20 条判据（A1–A18，含版面/位面/道具栏/手绘怪物，
...
npm run verify:all        # 以上四套，共 73 条判据
```
