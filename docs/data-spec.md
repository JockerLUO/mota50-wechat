...
2. **主包 4MB 限制**。`data/` 是纯文本且压缩率极高——**全部 JSON 合计约 177 KB，gzip 后仅约 27 KB**，
   打进主包完全没问题（距 4MB 上限有 140 倍余量）。但**图片资源**要放远程或分包。
   注意 12 种地形 + 34 只怪物 + 32 项道具的精灵图数量比 v0.1 的 3 层草案大一个量级，
   这才是真正会撑爆主包的部分。
   ⚠️ 实际进包的**不是整个 `data/`**，而是 `src/data/runtime-files.mjs` 清单里的 **58 个**
   （7 张顶层表 + 51 层地图，实测 177.2 KB）。`combat-cases.json`（黄金用例）与
   `monster-placement.json`（放置审计）是给 `npm run validate` / `npm run balance` 用的，
   游戏运行时一个字节都不需要。它们不进包靠的是**显式包含清单**，而不是排除清单 ——
   排除法会在「往 `data/` 里加一个工具用 json」时静默漏掉，包含清单不会。
...
4. **存档**用 `wx.setStorageSync`，单 key 上限 1MB。`history` 回退栈要限制深度（建议 50 步），
   否则长局游戏会写爆。
5. **代码包文件可以在运行期读**（官方「文件系统」权限表：代码包文件 *读=有*、*写=无*）。
   所以小游戏侧是 `wx.getFileSystemManager().readFileSync('data/floors/floor-00.json', 'utf8')`
   按需取数，而不是把 51 层全部内联进 `game.js`（那也正是 `npm run verify:minigame` 的
   「数据没有在构建期内联进入口」这条判据在守的东西）。
   两条硬要求：路径**从项目根目录写起**、**不支持 `./` `../` 前缀**
   （`a/b/c` 合法，`./a/b/c` 不合法）；也不用 `require('./data/x.json')` ——
   官方模块化文档只写了 `require` 加载 **js 模块**，json 走 require 属「社区在用、文档没背书」。
   两端差异的完整说明见 `docs/wechat-minigame.md` §7。
