"""
════════════════════════════════════════════════════════════════════
**这是「改前」的基线，不参与构建。**（2026-09-24 从 `tools/assetlib/bosses.py` 冻结过来）

冻结它的唯一理由：`verify_boss_art` 的门槛必须能**被证明会红**。
本项目的纪律是「阈值定在『改前必红、改后有余量』的位置」——
而 2026-09-24 这一版确实发现旧门槛（3.0 / 1.80 / 12% / 1100）在改前**一条都不红**，
即它是一条恒真的装饰性断言。抬高门槛之后，必须留下**能复现「旧素材会红」的东西**，
否则下一次「门槛是不是又变成装饰了」就只能靠猜。
...
它会把本文件以 `assetlib._baseline64` 的名字装进包里跑一遍（所以下面的相对
import 能解析），跑完即弃 —— 本文件**不在** `tools/assetlib/` 里，构建链
（`tools/assetlib/main.py`）永远不会 import 到它。
实测分布与门槛的推导写在 `tools/assetlib/bosses/common.py` 的门槛注释里。
════════════════════════════════════════════════════════════════════
